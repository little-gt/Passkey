...
![Passkey](https://img.shields.io/badge/Passkey-v1.1.3-007EC6?style=for-the-badge&logo=securityscorecard&logoColor=white)
...
| 版本 | 日期 | 说明 | 更新内容 |
|------|------|------|----------|
| v1.1.3 | 2026-07-01 | 多项质量更新 | 适配 MySQL、SQLite 和 PostgreSQL 数据库；优化在 BooAdmin 中的显示效果 |
| v1.1.2 | 2026-06-23 | 样式优化升级 | 暗色模式优化为黑色配色，减少视觉刺激，搭配 BooAdmin 使用体验更佳。 |
| v1.1.1 | 2026-06-04 | 支持切换平台验证器模式 | 新增"认证器类型限制"后台设置（允许所有验证器 / 仅允许平台验证器）、支持 Bitwarden / 1Password 等第三方密码管理器的 Passkey 功能、严格模式下强制锁定为平台验证器、安全模式与认证器类型限制联动控制（严格模式自动禁用选项并强制选中"仅平台验证器"，自定义模式恢复可用） |
| v1.1.0 | 2026-03-26 | 兼容 BooAdmin 暗色模式 | 在插件管理页面也可以体验原生的BooAdmin样式设计，一致性更强 |
| v1.0.6 | 2026-03-07 | 移动端兼容性全面优化 | 移动端响应式设计、iOS Safari/Android Chrome/Firefox 兼容性、智能浏览器检测、设备特定提示、触摸友好界面、渐进式降级、PWA 模式支持 |
| v1.0.5 | 2026-02-24 | 安全配置增强 | 三种预设安全级别（开发/常规/严格）、自定义安全参数调整、实时预览安全强度、移除不安全 Server 变量构造、强制从站点配置读取 siteUrl、增强域名格式验证防止 Host 头注入 |
| v1.0.4 | 2026-02-23 | 信息安全加固 | 全面信息脱敏（12 处敏感信息泄露）、统一错误处理机制（避免差异化攻击）、增强输入验证（防注入攻击）、优化错误日志记录（避免日志注入） |
| v1.0.3 | 2026-02-22 | 企业级安全解决方案 | ES256/RS256 签名验证（PHP OpenSSL 原生实现）、IEEE P1363 / DER 格式自动转换、Session 速率限制（防暴力破解）、Challenge 超时验证（防重放攻击）、签名计数器检测（防克隆认证器）、Origin 严格验证（防域名欺骗）、数据长度限制（防 DoS 攻击） |
| v1.0.2 | 2026-02-22 | 功能增强 | 登录历史记录查询、完整卸载支持、网页内通知系统 |
| v1.0.1 | - | 注册功能 | Passkey 新用户注册、注册表单弹窗（用户名/邮箱/昵称）、全局注册设置优先级控制 |
| v1.0.0 | - | 初始版本 | Passkey 注册和登录、后台管理界面、自动/手动注入模式、多设备支持 |
